        ...
        if not is_ffmpeg_available():
            ...
            try:
                ...
                opts = {
                    ...
                    'quiet': True, 'socket_timeout': SOCKET_TIMEOUT_SECONDS,
                    'http_headers': {'User-Agent': COMMON_USER_AGENT}
                }
                with yt_dlp.YoutubeDL(opts) as ydl:
    ...
    if do_transcript:
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(opts) as ydl:
                ...
                path = None
                for lang in ['en','ro']:
                    if subs.get(lang, {}).get('filepath'):
                        path = subs[lang]['filepath']; result['transcript_language'] = lang; break
            if not path:
                # fallback scan
                for lang in ['en','ro']:
                    p = os.path.join(TRANSCRIPTS_TEMP_DIR, f"{temp}.{lang}.vtt")
                    if os.path.exists(p): path, result['transcript_language'] = p, lang; break
            if path:
                text = vtt_to_plaintext(open(path, 'r', encoding='utf-8').read())
            ...
            for f in os.listdir(TRANSCRIPTS_TEMP_DIR):
                if f.startswith(temp): os.remove(os.path.join(TRANSCRIPTS_TEMP_DIR, f))
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=int(os.getenv('PORT', 5001)), debug=True)â
